package ly.stealth.metrics.kafka.statsd
 ...
 */
class StatsD(context: ActorContext = null,
             host: String,
             port: Int,
  ...
  if (context != null)
    actorRef = context.actorOf(Props(new StatsDActor(host, port, multiMetrics, packetBufferSize)))
  else
    actorRef = ActorSystem.create("statsd").actorOf(Props(new StatsDActor(host, port, multiMetrics, packetBufferSize)))
   ...
   */
  def gauge(key: String, value: Double = 1.0, sampleRate: Double = 1.0) = {
    send(key, value.toString, StatsDProtocol.GAUGE_METRIC, sampleRate)
  }
